...
template <typename T>
inline std::array<float, 3> to_arr3f(T &p)
{
  ...
  while (lasreader->read_point())
  {
    if (lasreader->point.get_classification() == 2)
    {
      ...
      if (i++ % thin_nth == 0)
      {
        ...
        points.push_back({float(lasreader->point.get_x()),
  ...
  while (lasreader->read_point())
  {
    if (lasreader->point.get_classification() == 2)
    {
      ...
      if (i++ % thin_nth == 0)
      {
        ...
        if (overwritez)
        {
          points.push_back({float(lasreader->point.get_x() - offset[0]),
...
void TinSimpNode::process()
{
  ...
  if (create_triangles)
  {
         ...
         ++fit)
    {
      ...
      triangles.push_back({to_arr3f<tinsimp::Point>(p0),
...
void CDT2TrianglesNode::process()
{
       ...
       ++fit)
  {
    ...
    auto &p1 = fit->vertex(1)->point();
    auto &p2 = fit->vertex(2)->point();
    triangles.push_back({to_arr3f<Point>(p0),
                         to_arr3f<Point>(p1),
                         to_arr3f<Point>(p2)});
  }
  cdt.clear();
